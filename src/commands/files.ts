...
async function deleteRemote(profile: RockProfile, remotePath: string): Promise<ActionResponse> {
...
export function makeFilesCommand(): Command {
	...
	files
		.command("ls")
		.description(
			"List a directory on the Rock server (root: '/Plugins', '/Themes', '/Content', '/App_Data')",
		)
	...
	files
		...
		.action(async (local: string, remote: string, opts: { profile?: string }) => {
			...
			if (!result.ActionSuccessful) {
				throw new Error(`Upload failed: ${result.ResponseMessage || "(no message from server)"}`);
			}
			process.stderr.write(
	...
	files
		...
		.action(async (path: string, opts: { profile?: string }) => {
			...
			if (!result.ActionSuccessful) {
				throw new Error(`mkdir failed: ${result.ResponseMessage || "(no message from server)"}`);
			}
